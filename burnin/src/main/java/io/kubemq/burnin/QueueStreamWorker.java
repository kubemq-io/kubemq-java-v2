// Queue Stream worker: sendQueueMessage for send, receiveQueueMessages for receive.
// autoAckMessages=false, visibilitySeconds for manual ack. Per-message ack.
...
/**
 * Queue Stream pattern worker: poll-based queue with manual batch ack.
 * v2: Constructed per-channel by PatternGroup.
 */
public final class QueueStreamWorker extends BaseWorker {

    private static final Logger logger = Logger.getLogger(QueueStreamWorker.class.getName());
    private static final String SDK = "java";
    private static final String PATTERN_NAME = "queue_stream";
    ...
    public void startConsumers(PubSubClient pubSubClient, QueuesClient queuesClient, CQClient cqClient) {
        for (int i = 0; i < consumersPerChannel; i++) {
            ...
            Thread t = new Thread(() -> runConsumer(consumerId, queuesClient),
                    "queue-stream-consumer-" + getChannelIndex() + "-" + i);
            t.setDaemon(true);
    ...
    private void runConsumer(String consumerId, QueuesClient client) {
        ...
        int visibilitySec = getConfig().getQueue().getVisibilitySeconds();
        ...
        while (!consumerStop.get()) {
            try {
                QueuesPollRequest request = QueuesPollRequest.builder()
                        ...
                        .visibilitySeconds(visibilitySec)
                        .build();

                QueuesPollResponse response = client.receiveQueueMessages(request);

                if (response.isError()) {
                    String errMsg = response.getError() != null ? response.getError() : "unknown downstream error";
                ...
                boolean isServerClose = msg.toLowerCase().contains("server closed");
                if (isServerClose) {
                    incReconnection();
                    continue;
                }

                System.err.println("queue_stream consumer error: " + msg);
                recordError("receive_failure");
                incReconnection();
                sleepQuietly(1000);
            }
    ...
    public void startProducers(PubSubClient pubSubClient, QueuesClient queuesClient, CQClient cqClient) {
        for (int i = 0; i < producersPerChannel; i++) {
            ...
            Thread t = new Thread(() -> runProducer(producerId, queuesClient),
                    "queue-stream-producer-" + getChannelIndex() + "-" + i);
            t.setDaemon(true);
            ...
            try {
                ...
                client.sendQueuesMessageAsync(message)
                        .whenComplete((result, ex) -> {
                            inFlight.release();
                            double durationSec = (System.nanoTime() - t0) / 1_000_000_000.0;
                            Metrics.observeSendDuration(PATTERN_NAME, durationSec);
