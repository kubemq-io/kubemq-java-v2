...
class AuthSecurityTest {
  ...
  class TokenAuthenticationTests {
    ...
    void testTokenUpdateThreadSafe() throws Exception {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("thread-safe-test").build();
    ...
    void testNullTokenSkipsHeader() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("null-token-test").build();
    ...
    void testUnauthenticatedWithNoTokenGivesSetTokenHint() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("no-token-hint-test").build();
    ...
    void testNonUnauthenticatedNotWrappedAsAuth() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("other-status-test").build();
  ...
  class TlsEncryptionTests {
    ...
    void testTlsDefaultsToFalseForLocalhost() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("local-tls-test").build();
    ...
    void testTlsHandshakeCertFailure() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("tls-classify-test").build();

      SSLHandshakeException sslEx = new SSLHandshakeException("certificate has expired");
    ...
    void testTlsHandshakeProtocolFailure() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("tls-protocol-test").build();

      SSLHandshakeException sslEx = new SSLHandshakeException("no common protocol");
      RuntimeException classified = client.classifyTlsException(sslEx);
    ...
    void testTlsNetworkError() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("tls-network-test").build();

      SSLException sslEx = new SSLException("connection reset", new IOException("reset by peer"));
      RuntimeException classified = client.classifyTlsException(sslEx);
    ...
    void testGenericTlsFailure() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("tls-generic-test").build();
  ...
  class CredentialProviderTests {
    ...
    void testProactiveRefreshScheduled() throws Exception {
      ...
      CredentialProvider provider =
          () -> {
            int count = callCount.incrementAndGet();
            return new TokenResult("token-v" + count, Instant.now().plusMillis(200));
          };
    ...
    void testCredentialManagerRequiresProvider() {
      assertThrows(IllegalArgumentException.class, () -> new CredentialManager(null, null));
    }
  ...
  class SecurityBestPracticesTests {
    ...
    void testKubeMQClientToStringExcludesAuthToken() {
      ...
      assertFalse(str.contains("secret-token-xyz"), "toString must not contain the token value");
      assertTrue(str.contains("token_present=true"), "toString must contain token_present=true");
    ...
    void testKubeMQClientToStringNoTokenShowsFalse() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("no-token-tostring").build();
    ...
    void testToStringIncludesConfig() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("config-tostring").build();
  ...
  class ConfigurationExceptionTests {
    ...
    void testConfigurationExceptionCode() {
      ConfigurationException ex =
          ConfigurationException.builder().message("TLS config error").build();
    ...
    void testConfigurationExceptionCause() {
      ...
      ConfigurationException ex =
          ConfigurationException.builder().message("TLS failed").cause(cause).build();
