...
/**
 * Unit tests for EventStreamHelper - covers StreamObserver callbacks and message sending. Updated
 * to work with per-request futures pattern (CRITICAL-1 fix).
 */
...
class EventStreamHelperTest {
  ...
  class SendEventMessageTests {
    ...
    void sendEventMessage_createsStreamOnFirstCall() {
      ...
      Kubemq.Event event =
          Kubemq.Event.newBuilder().setEventID("event-1").setChannel("test-channel").build();
    ...
    void sendEventMessage_reusesExistingStream() {
      ...
      Kubemq.Event event2 =
          Kubemq.Event.newBuilder().setEventID("event-2").setChannel("test-channel").build();
  ...
  class SendEventStoreMessageTests {
    ...
    void sendEventStoreMessage_createsStreamOnFirstCall() throws Exception {
      ...
      CompletableFuture<EventSendResult> resultFuture =
      ...
      Kubemq.Result result =
          Kubemq.Result.newBuilder().setEventID(actualEventId).setSent(true).build();
