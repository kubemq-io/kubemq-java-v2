...
class BatchQueueSendTest {
  ...
  void sendQueuesMessages_emptyList_throwsValidationException() {
    ValidationException ex =
        assertThrows(ValidationException.class, () -> client.sendQueuesMessages(new ArrayList<>()));
    assertTrue(ex.getMessage().contains("null or empty"));
  }
  ...
  void sendQueuesMessages_invalidMessage_throwsValidationException() {
    QueueMessage valid = QueueMessage.builder().channel("queue-1").body("body".getBytes()).build();
    QueueMessage invalid = QueueMessage.builder().channel(null).body("body".getBytes()).build();
  ...
  void sendQueuesMessages_emptyBodyAndMetadata_throwsValidationException() {
    QueueMessage emptyMsg = QueueMessage.builder().channel("queue-1").build();

    assertThrows(ValidationException.class, () -> client.sendQueuesMessages(List.of(emptyMsg)));
  }
}
