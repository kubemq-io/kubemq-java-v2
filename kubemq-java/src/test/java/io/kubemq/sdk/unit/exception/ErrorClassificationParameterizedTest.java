...
/**
 * Parameterized test for gRPC status code → SDK error classification.
 * Uses @EnumSource to verify all 17 gRPC status codes (0-16) are mapped
 * to correct ErrorCategory and retryable flag per REQ-ERR-6.
 *
 * Complements the per-code assertions in GrpcErrorMapperTest by providing
 * a single parameterized sweep that catches missed codes automatically.
 */
...
class ErrorClassificationParameterizedTest {
    ...
    @EnumSource(value = Status.Code.class, mode = EnumSource.Mode.EXCLUDE, names = {"OK"})
    void grpcStatusCode_mapsToNonNullCategoryWithCorrectRetryable(Status.Code code) {
        StatusRuntimeException grpcError = Status.fromCode(code)
            .withDescription("test error for " + code)
            .asRuntimeException();

        KubeMQException exception = GrpcErrorMapper.map(
            grpcError, "testOp", "testChannel", "req-1", false);
    ...
    void grpcStatusCode_preservesOriginalExceptionInCause(Status.Code code) {
        StatusRuntimeException grpcError = Status.fromCode(code)
            .withDescription("cause-chain test")
            .asRuntimeException();

        KubeMQException exception = GrpcErrorMapper.map(
            grpcError, "testOp", null, null, false);

        assertSame(grpcError, exception.getCause(),
            "Original gRPC exception must be preserved for " + code);
    }
    ...
    void grpcStatusCode_numericValueMatchesGrpcCode(Status.Code code) {
        StatusRuntimeException grpcError = Status.fromCode(code)
            .withDescription("numeric test")
            .asRuntimeException();

        KubeMQException exception = GrpcErrorMapper.map(
            grpcError, "testOp", null, null, false);

        assertEquals(code.value(), exception.getStatusCode(),
    ...
    void grpcStatusCode_contextFieldsPropagated(Status.Code code) {
        StatusRuntimeException grpcError = Status.fromCode(code)
            .withDescription("context test")
            .asRuntimeException();

        KubeMQException exception = GrpcErrorMapper.map(
            grpcError, "myOp", "myChannel", "myReq", false);
