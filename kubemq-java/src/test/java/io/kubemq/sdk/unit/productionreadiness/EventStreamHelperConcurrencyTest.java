 ...
 *
 * <p>The EventStreamHelper now uses a ConcurrentHashMap (pendingResponses) with per-request
 * CompletableFutures instead of a single shared future. This ensures thread-safe concurrent access
 * where each request gets its own response.
 *
...
class EventStreamHelperConcurrencyTest {
  ...
  @Test
  @DisplayName(
      "CRITICAL-1 FIX: Concurrent sendEventStoreMessage calls each receive their own response")
  void concurrentSendEventStoreMessage_shouldReceiveCorrectResponses() throws Exception {
    ...
    for (String eventId : sentEventIds) {
      Kubemq.Result result = Kubemq.Result.newBuilder().setEventID(eventId).setSent(true).build();
    ...
    assertEquals(
        ...
        "All "
            + numThreads
            + " threads should receive a response. Got "
            + successCount
            + " successes and "
            + timeoutCount
            + " timeouts.");
  ...
  void secondConcurrentCall_shouldNotOverwriteFirstFuture() throws Exception {
    ...
    assertEquals(1, thread1Result.get(), "Thread 1 should receive a valid response");
    assertEquals(1, thread2Result.get(), "Thread 2 should receive a valid response");
  }
  ...
  void verifyPendingResponsesUsesPerRequestFutures() throws Exception {
    ...
    Map<String, CompletableFuture<EventSendResult>> pendingResponses =
        (Map<String, CompletableFuture<EventSendResult>>)
            pendingResponsesField.get(eventStreamHelper);

    assertNotNull(pendingResponses, "pendingResponses map should exist");
    assertTrue(
        pendingResponses instanceof ConcurrentHashMap,
  ...
  void verifyRequestTimestampsExists() throws Exception {
    ...
    assertTrue(
        requestTimestamps instanceof ConcurrentHashMap,
  ...
  void responsesAreMatchedByEventId() throws Exception {
    ...
    for (int i = sentEventIds.size() - 1; i >= 0; i--) {
      Kubemq.Result result =
          Kubemq.Result.newBuilder().setEventID(sentEventIds.get(i)).setSent(true).build();
      capturedResultObserver.onNext(result);
    }
