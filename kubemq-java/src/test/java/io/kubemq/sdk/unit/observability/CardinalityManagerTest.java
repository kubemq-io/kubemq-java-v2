...
class CardinalityManagerTest {
    ...
    void belowThreshold() {
        KubeMQMetrics metrics = new KubeMQMetrics(meterProvider, "1.0",
                new CardinalityConfig(3, null));
        ...
        Optional<MetricData> counter = data.stream()
                .filter(m -> m.getName().equals("messaging.client.sent.messages"))
                .findFirst();
        assertTrue(counter.isPresent());

        long distinctChannels = counter.get().getData().getPoints().stream()
                .map(p -> p.getAttributes().get(
    ...
    void aboveThreshold() {
        ...
        long pointsWithChannel = counter.get().getData().getPoints().stream()
                .filter(p -> p.getAttributes().get(
                        io.opentelemetry.api.common.AttributeKey.stringKey("messaging.destination.name")) != null)
                .count();
        assertEquals(2, pointsWithChannel,
    ...
    void allowlistAlwaysIncluded() {
        ...
        KubeMQMetrics metrics = new KubeMQMetrics(meterProvider, "1.0",
                new CardinalityConfig(1, allowlist));
        metrics.recordSentMessage("publish", "ch-1");
        metrics.recordSentMessage("publish", "ch-2");
        ...
        boolean hasImportantChannel = counter.get().getData().getPoints().stream()
                .anyMatch(p -> "important-channel".equals(
                        p.getAttributes().get(
                                io.opentelemetry.api.common.AttributeKey.stringKey("messaging.destination.name"))));
        assertTrue(hasImportantChannel,
                "Allowlisted channel should always be included");
    }
    ...
    void allowlistUnmodifiable() {
        ...
        assertThrows(UnsupportedOperationException.class,
                () -> config.getChannelAllowlist().add("ch-2"));
    }
}
