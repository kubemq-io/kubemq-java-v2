...
class QueuesPollResponseTest {
  ...
  class BuilderTests {
    ...
    void builder_withAllFields_createsResponse() {
      List<QueueMessageReceived> messages =
          Arrays.asList(createMockMessage("msg-1"), createMockMessage("msg-2"));
    ...
    void builder_withMinimalFields_createsResponse() {
      QueuesPollResponse response =
          QueuesPollResponse.builder().refRequestId("ref-minimal").build();
    ...
    void builder_withNoMessages_createsEmptyList() {
      QueuesPollResponse response = QueuesPollResponse.builder().refRequestId("ref-empty").build();
  ...
  class GetterSetterTests {
    ...
    void setMessages_updatesMessages() {
      QueuesPollResponse response = new QueuesPollResponse();
      List<QueueMessageReceived> messages = Arrays.asList(createMockMessage("msg-setter"));
  ...
  class IsErrorTests {
    ...
    void isError_returnsTrueWhenSet() {
      QueuesPollResponse response =
          QueuesPollResponse.builder().isError(true).error("Some error").build();
      assertTrue(response.isError());
    }

    @Test
    void isError_methodReturnsCorrectValue() {
      QueuesPollResponse responseWithError = QueuesPollResponse.builder().isError(true).build();
      QueuesPollResponse responseWithoutError = QueuesPollResponse.builder().isError(false).build();
  ...
  class TransactionOperationValidationTests {
    ...
    void ackAll_withAutoAcked_throwsException() {
      ...
      IllegalStateException ex = assertThrows(IllegalStateException.class, response::ackAll);
    ...
    void rejectAll_withAutoAcked_throwsException() {
      ...
      IllegalStateException ex = assertThrows(IllegalStateException.class, response::rejectAll);
      assertTrue(ex.getMessage().contains("auto ack"));
    }
    ...
    void reQueueAll_withAutoAcked_throwsException() {
      ...
      IllegalStateException ex =
          assertThrows(IllegalStateException.class, () -> response.reQueueAll("other-channel"));
      assertTrue(ex.getMessage().contains("auto ack"));
    }
  ...
  class ToStringTests {
    ...
    void toString_includesAllFields() {
      List<QueueMessageReceived> messages = Arrays.asList(createMockMessage("msg-str"));
  ...
  class CompleteMethodTests {
    ...
    void complete_withAutoAcked_doesNotThrow() throws Exception {
      ...
      QueuesPollResponse response =
          QueuesPollResponse.builder().messages(Arrays.asList(msg)).isAutoAcked(true).build();

      // Use reflection to call complete with a lambda that does nothing
      Method completeMethod =
          QueuesPollResponse.class.getDeclaredMethod(
              "complete", Class.forName("io.kubemq.sdk.queues.RequestSender"));
  ...
  class TransactionOperationSuccessTests {
    ...
    private void setupResponseWithSender(
        QueuesPollResponse response,
  ...
  class MessageCompletionTrackingTests {
    ...
    void markMessageCompleted_singleMessageCompletesTransaction() throws Exception {
      ...
      QueuesPollResponse response =
          QueuesPollResponse.builder().messages(Arrays.asList(msg)).isAutoAcked(false).build();
      setupResponseWithSender(response);
    ...
    void markMessageCompleted_handlesUnknownMessageId() {
      QueuesPollResponse response =
          QueuesPollResponse.builder().messages(new ArrayList<>()).isAutoAcked(false).build();
