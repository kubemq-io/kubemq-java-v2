...
class MetricsTest {
    ...
    class OTelMetricsTests {
        ...
        void operationDuration() {
            ...
            Optional<MetricData> histogram = data.stream()
        ...
        void retryAttempts() {
            ...
            Collection<MetricData> data = reader.collectAllMetrics();
            Optional<MetricData> counter = data.stream()
                    .filter(m -> m.getName().equals("kubemq.client.retry.attempts"))
                    .findFirst();
        ...
        void instrumentationScope() {
            ...
            Collection<MetricData> data = reader.collectAllMetrics();
            Optional<MetricData> metric = data.stream()
                    .filter(m -> m.getName().equals("messaging.client.sent.messages"))
                    .findFirst();
            assertTrue(metric.isPresent());
            assertEquals("io.kubemq.sdk",
                    metric.get().getInstrumentationScopeInfo().getName());
            assertEquals("2.1.1",
                    metric.get().getInstrumentationScopeInfo().getVersion());
        }
    }
