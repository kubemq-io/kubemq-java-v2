...
class QueueSendResultTest {
  ...
  private static final long EXPIRED_AT_NANOS =
      1703548800L * 1_000_000_000L; // 2023-12-26 00:00:00 UTC
  private static final long DELAYED_TO_NANOS =
      1703466000L * 1_000_000_000L; // 2023-12-25 01:00:00 UTC
  ...
  class DecodeTests {
    ...
    void decode_withAllFields_decodesCorrectly() {
      SendQueueMessageResult protoResult =
          SendQueueMessageResult.newBuilder()
    ...
    void decode_withNullMessageId_usesEmptyString() {
      SendQueueMessageResult protoResult =
          SendQueueMessageResult.newBuilder().setSentAt(SENT_AT_NANOS).setIsError(false).build();
    ...
    void decode_withNullError_usesEmptyString() {
      SendQueueMessageResult protoResult =
          SendQueueMessageResult.newBuilder().setMessageID("msg-000").setIsError(true).build();
  ...
  class BuilderTests {
    ...
    void builder_withMinimalFields_createsResult() {
      QueueSendResult result = QueueSendResult.builder().id("minimal-123").isError(false).build();
  ...
  class ToStringTests {
    ...
    void toString_withNullDates_handlesGracefully() {
      QueueSendResult result = QueueSendResult.builder().id("null-dates").isError(false).build();
