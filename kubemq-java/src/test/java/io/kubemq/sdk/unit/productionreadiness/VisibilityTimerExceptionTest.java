 ...
 *
 * <p>The QueueMessageReceived now uses a shared ScheduledExecutorService instead of Timer per
 * message, and the onVisibilityExpired() method NO LONGER throws an exception. Instead, it logs the
 * error and continues gracefully.
 *
 * <p>These tests verify the fix works correctly.
 */
@ExtendWith(MockitoExtension.class)
class VisibilityTimerExceptionTest {

  @Mock private QueuesPollResponse mockPollResponse;
  ...
  void onVisibilityExpired_shouldNotThrowException() throws Exception {
    ...
    Method onVisibilityExpiredMethod =
        QueueMessageReceived.class.getDeclaredMethod("onVisibilityExpired");
    onVisibilityExpiredMethod.setAccessible(true);

    // CRITICAL-2 FIX VERIFICATION: The method should NOT throw an exception
    assertDoesNotThrow(
        () -> onVisibilityExpiredMethod.invoke(message),
  ...
  void usesScheduledExecutorServiceInsteadOfTimer() throws Exception {
    ...
    Field visibilityExecutorField =
        QueueMessageReceived.class.getDeclaredField("VISIBILITY_EXECUTOR");
    visibilityExecutorField.setAccessible(true);

    Object executor = visibilityExecutorField.get(null); // static field
    assertNotNull(executor, "VISIBILITY_EXECUTOR should exist");
    assertTrue(
  ...
  void staticExecutorIsSharedAcrossMessages() {
    ...
    assertTrue(
        executor instanceof ScheduledExecutorService, "Should be a ScheduledExecutorService");
  }
  ...
  void stateAfterTimerException_isConsistent() throws Exception {
    ...
    IllegalStateException ex =
        assertThrows(
            IllegalStateException.class, message::ack, "Ack should fail after visibility expired");

    assertTrue(
        ex.getMessage().contains("completed") || ex.getMessage().contains("Transaction"),
        "Exception should indicate transaction is already completed");
  }

  // Helper methods

  private QueueMessageReceived createMessageWithShortVisibility(int visibilitySeconds)
      throws Exception {
  ...
  private void setRequestSenderViaReflection(
      QueueMessageReceived message, Consumer<Kubemq.QueuesDownstreamRequest> senderAction)
      throws Exception {
    ...
    Method setterMethod =
        QueueMessageReceived.class.getDeclaredMethod("setRequestSender", requestSenderClass);
    setterMethod.setAccessible(true);
    setterMethod.invoke(message, proxy);
  }

  private void setPollResponse(QueueMessageReceived message, QueuesPollResponse response)
      throws Exception {
