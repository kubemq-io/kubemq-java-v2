...
class ErrorMessageBuilderTest {
    ...
    void build_withChannelIncludesChannelInMessage() {
        String msg = ErrorMessageBuilder.build(
    ...
    void build_withoutChannelOmitsChannelClause() {
        String msg = ErrorMessageBuilder.build(
            "ping", null, "Server unreachable", ErrorCode.CONNECTION_FAILED);
    ...
    void build_includesSuggestionForKnownCodes() {
        String msg = ErrorMessageBuilder.build(
            "subscribe", "events", "Authentication failed",
    ...
    void build_noSuggestionForUnknownCode() {
        String msg = ErrorMessageBuilder.build(
            "op", null, "something", ErrorCode.UNKNOWN_ERROR);
    ...
    void withRetryContext_appendsRetryInfo() {
        ...
        String result = ErrorMessageBuilder.withRetryContext(
            base, 3, 5, Duration.ofMillis(1500));
    ...
    void noSuggestion_forCodesWithoutSuggestions() {
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.HANDLER_ERROR).contains("Suggestion:"));
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.CANCELLED_BY_CLIENT).contains("Suggestion:"));
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.CANCELLED_BY_SERVER).contains("Suggestion:"));
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.PARTIAL_FAILURE).contains("Suggestion:"));
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.RETRY_THROTTLED).contains("Suggestion:"));
        assertFalse(ErrorMessageBuilder.build("op", null, "err", ErrorCode.ABORTED).contains("Suggestion:"));
    }

    @Test
    void build_messageFormatMatchesSpec() {
        String msg = ErrorMessageBuilder.build(
            "SendEvent", "orders.created", "UNAVAILABLE: server shutting down",
