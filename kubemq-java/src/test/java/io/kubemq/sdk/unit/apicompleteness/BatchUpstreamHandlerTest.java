...
class BatchUpstreamHandlerTest {
    ...
    void sendQueuesMessages_encodesAllInOneRequest() throws Exception {
        ...
        QueueMessage msg1 = QueueMessage.builder()
                .channel("queue-1")
                .body("body-1".getBytes())
                .build();
        QueueMessage msg2 = QueueMessage.builder()
                .channel("queue-2")
                .body("body-2".getBytes())
                .build();

        CompletableFuture<List<QueueSendResult>> resultFuture = CompletableFuture.supplyAsync(() ->
                handler.sendQueuesMessages(Arrays.asList(msg1, msg2))
        );
    ...
    void sendQueuesMessages_handlesErrorResponse() throws Exception {
        ...
        CompletableFuture<List<QueueSendResult>> resultFuture = CompletableFuture.supplyAsync(() ->
                handler.sendQueuesMessages(List.of(msg1))
        );
