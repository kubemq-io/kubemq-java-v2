...
class ConnectionConfigTest {
  ...
  class DefaultAddressTests {
    ...
    void nullAddress_usesDefault() {
      CQClient client = CQClient.builder().address(null).clientId("test").build();
    ...
    void emptyAddress_usesDefault() {
      CQClient client = CQClient.builder().address("").clientId("test").build();
      assertEquals("localhost:50000", client.getAddress());
      client.close();
    }
    ...
    void customAddress_isRespected() {
      CQClient client =
          CQClient.builder().address("myserver:9090").clientId("test").tls(false).build();
  ...
  class GracefulShutdownTests {
    ...
    void operationAfterClose_throwsClientClosedException() {
      CQClient client = CQClient.builder().address("localhost:50000").clientId("test").build();
      client.close();
      assertThrows(ClientClosedException.class, () -> client.sendCommandRequest(null));
    }
