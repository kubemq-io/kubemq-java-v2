...
class RetryExecutorTest {
  ...
  class SuccessfulExecutionTests {
    ...
    void execute_successOnFirstAttempt() {
      ...
      String result = executor.execute(() -> "hello", "testOp", "ch1", OperationSafety.SAFE);
              ...
              () ->
                  executor.execute(
                      () -> {
                        ...
                        throw AuthenticationException.builder().message("Invalid token").build();
                      },
                      "testOp",
                      "ch1",
                      OperationSafety.SAFE));
      ...
      assertThrows(
          ...
          () ->
              executor.execute(
                  () -> {
                    attempts.incrementAndGet();
                    throw ValidationException.builder().message("Bad input").build();
                  },
                  "testOp",
                  "ch1",
                  OperationSafety.SAFE));
      ...
      assertThrows(
          ...
          () ->
              executor.execute(
                  () -> {
                    attempts.incrementAndGet();
                    throw ConnectionException.builder().message("fail").retryable(true).build();
                  },
                  "op",
                  "ch",
                  OperationSafety.SAFE));
  ...
  class ConnectionReadinessTests {
    ...
    void connectionNotReady_stopsRetrying() {
      AtomicInteger attempts = new AtomicInteger(0);
      RetryExecutor executor = new RetryExecutor(fastPolicy(), () -> false);
    ...
    void connectionReady_allowsRetry() {
      AtomicInteger attempts = new AtomicInteger(0);
      RetryExecutor executor = new RetryExecutor(fastPolicy(), () -> true);
