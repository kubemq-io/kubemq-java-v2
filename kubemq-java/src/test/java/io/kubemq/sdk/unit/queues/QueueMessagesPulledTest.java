...
class QueueMessagesPulledTest {
  ...
  class BuilderTests {
    ...
    void builder_withError_setsErrorFields() {
      QueueMessagesPulled pulled =
          QueueMessagesPulled.builder().isError(true).error("Queue not found").build();
    ...
    void builder_withMessages_setsMessages() {
      ...
      QueueMessagesPulled pulled = QueueMessagesPulled.builder().messages(messages).build();
  ...
  class GetterSetterTests {
    ...
    void builder_withIsError_setsTheFlag() {
      QueueMessagesPulled pulled = QueueMessagesPulled.builder().isError(true).build();
      assertTrue(pulled.isError());
    }
    ...
    void builder_withError_setsTheMessage() {
      QueueMessagesPulled pulled = QueueMessagesPulled.builder().error("New error").build();
    ...
    void messagesList_isModifiable() {
      ...
      pulled.getMessages().add(QueueMessageWaitingPulled.builder().id("added-msg").build());
  ...
  class MultipleMessagesTests {
    ...
    void canHoldMultipleMessages() {
      ...
      for (int i = 0; i < 10; i++) {
        pulled.getMessages().add(QueueMessageWaitingPulled.builder().id("msg-" + i).build());
      }
