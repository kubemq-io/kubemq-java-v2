...
class TracingTest {
  ...
  class NoOpTests {
    ...
    void startSpanReturnsNonNull() {
      Object span = NoOpTracing.INSTANCE.startSpan(null, "publish", "test-channel", "msg-1", null);
      assertNotNull(span);
    }
    ...
    void linkedConsumerSpanReturnsNonNull() {
      Object span = NoOpTracing.INSTANCE.startLinkedConsumerSpan("process", "ch", "msg-1", null);
      assertNotNull(span);
    }
    ...
    void batchReceiveReturnsNonNull() {
      Object span = NoOpTracing.INSTANCE.startBatchReceiveSpan("ch", 5, Collections.emptyList());
      assertNotNull(span);
    }
  }
  ...
  class OTelTracingTests {
    ...
    void producerSpanAttributes() {
      Object spanObj =
          tracing.startSpan(SpanKind.PRODUCER, "publish", "events-channel", "msg-123", null);
      ...
      assertEquals(
          "publish",
          span.getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.stringKey("messaging.operation.name")));
      assertEquals(
          "events-channel",
          span.getAttributes()
              .get(
      ...
      assertEquals(
          "test-client",
          span.getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.stringKey("messaging.client.id")));
      assertEquals(
          "localhost",
          span.getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.stringKey("server.address")));
      assertEquals(
          50000L,
          span.getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.longKey("server.port")));
    }

    @Test
    @DisplayName("startSpan creates CLIENT span for commands")
    void clientSpanForCommands() {
      Object spanObj = tracing.startSpan(SpanKind.CLIENT, "send", "cmd-channel", "req-1", null);
      tracing.endSpan(spanObj);
    ...
    void setErrorOnSpan() {
      Object spanObj = tracing.startSpan(SpanKind.PRODUCER, "publish", "ch", null, null);
      ...
      assertTrue(span.getEvents().stream().anyMatch(e -> e.getName().equals("exception")));
    }
    ...
    void retryEvent() {
      ...
      assertEquals(
          ...
          events
              .get(0)
              .getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.longKey("retry.attempt")));
      assertEquals(
          1.5,
          events
              .get(0)
              .getAttributes()
              .get(io.opentelemetry.api.common.AttributeKey.doubleKey("retry.delay_seconds")));
    }

    @Test
    @DisplayName("recordDlqEvent adds DLQ span event")
    void dlqEvent() {
      Object spanObj = tracing.startSpan(SpanKind.CONSUMER, "settle", "ch", null, null);
      tracing.recordDlqEvent(spanObj);
      tracing.endSpan(spanObj);

      SpanData span = exporter.getFinishedSpanItems().get(0);
      assertTrue(
          span.getEvents().stream().anyMatch(e -> e.getName().equals("message.dead_lettered")));
    }
    ...
    void consumerGroup() {
      Object spanObj = tracing.startSpan(SpanKind.CONSUMER, "process", "ch", null, null);
  ...
  class FactoryTests {
    ...
    void createsOTelTracing() {
      ...
      try {
        Tracing tracing = TracingFactory.create(provider, "1.0.0", "client-1", "localhost", 50000);
    ...
    void createsWithNullProvider() {
      Tracing tracing = TracingFactory.create(null, "1.0.0", "client-1", "localhost", 50000);
      assertNotNull(tracing);
      assertFalse(tracing instanceof NoOpTracing);
    }
  }
