...
class QueueMessagesWaitingTest {
  ...
  class BuilderTests {
    ...
    void builder_withError_setsErrorFields() {
      QueueMessagesWaiting waiting =
          QueueMessagesWaiting.builder().isError(true).error("Queue not found").build();
    ...
    void builder_withMessages_setsMessages() {
      ...
      messages.add(QueueMessageWaitingPulled.builder().id("msg-1").channel("test-queue").build());

      QueueMessagesWaiting waiting = QueueMessagesWaiting.builder().messages(messages).build();
  ...
  class GetterSetterTests {
    ...
    void builder_withIsError_setsTheFlag() {
      QueueMessagesWaiting waiting = QueueMessagesWaiting.builder().isError(true).build();
      assertTrue(waiting.isError());
    }
    ...
    void builder_withError_setsTheMessage() {
      QueueMessagesWaiting waiting = QueueMessagesWaiting.builder().error("New error").build();
    ...
    void messagesList_isModifiable() {
      ...
      waiting.getMessages().add(QueueMessageWaitingPulled.builder().id("added-msg").build());
  ...
  class MultipleMessagesTests {
    ...
    void canHoldMultipleMessages() {
      ...
      for (int i = 0; i < 10; i++) {
        waiting.getMessages().add(QueueMessageWaitingPulled.builder().id("msg-" + i).build());
      }
