...
class GrpcErrorMapperTest {
    ...
    class StatusCodeMappingTests {
        ...
        void ok_throwsIllegalArgument() {
            assertThrows(IllegalArgumentException.class, () ->
                GrpcErrorMapper.map(
                    grpcError(Status.OK), "ping", null, null, false));
        }

        @Test
        void unavailable_mapsToConnectionException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "sendEvent", "ch1", "req1", false);
        ...
        void notFound_mapsToKubeMQExceptionWithNotFoundCategory() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.NOT_FOUND), "subscribe", "missing", null, false);
        ...
        void alreadyExists_mapsToValidationException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.ALREADY_EXISTS), "createChannel", "ch", null, false);
        ...
        void resourceExhausted_mapsToThrottlingException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.RESOURCE_EXHAUSTED), "sendBatch", "ch", null, false);
        ...
        void failedPrecondition_mapsToValidationException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.FAILED_PRECONDITION), "op", "ch", null, false);
        ...
        void aborted_mapsToConnectionException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.ABORTED), "op", "ch", null, false);
        ...
        void outOfRange_mapsToValidationException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.OUT_OF_RANGE), "op", null, null, false);
        ...
        void unimplemented_mapsToServerException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNIMPLEMENTED), "op", null, null, false);
        ...
        void internal_mapsToServerException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.INTERNAL), "op", null, null, false);
        ...
        void dataLoss_mapsToServerException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.DATA_LOSS), "op", null, null, false);
        ...
        void unknown_mapsToConnectionExceptionWithRetry() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNKNOWN), "op", null, null, false);
    ...
    class CancelledStatusTests {
        ...
        void cancelled_withLocalContext_mapsToOperationCancelled() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.CANCELLED), "sendEvent", "ch", null, true);
        ...
        void cancelled_withoutLocalContext_mapsToConnectionException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.CANCELLED), "sendEvent", "ch", null, false);
    ...
    class CauseChainTests {
        ...
        void message_containsOperationAndDescription() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "sendEvent", "orders", null, false);
    ...
    class ContextPropagationTests {
        ...
        void operation_isSetOnMappedException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "subscribe", "events", null, false);
        ...
        void channel_isSetOnMappedException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "op", "my-channel", null, false);
        ...
        void requestId_isSetOnMappedException() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "op", null, "req-abc", false);
        ...
        void statusCode_matchesGrpcCode() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.UNAVAILABLE), "op", null, null, false);
        ...
        void nullChannel_handledGracefully() {
            KubeMQException result = GrpcErrorMapper.map(
                grpcError(Status.INTERNAL), "ping", null, null, false);
