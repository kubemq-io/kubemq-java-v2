...
class KubeMQClientChannelTest {
  ...
  @Mock private ManagedChannel mockManagedChannel;
  ...
  class PingErrorTests {
    ...
    void ping_serverUnavailable_throwsRuntimeException() {
      ...
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("ping-error-test").build();

      // Replace the blocking stub with mock
      when(mockBlockingStub.ping(any()))
          .thenThrow(
    ...
    void ping_deadlineExceeded_throwsRuntimeException() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("ping-timeout-test").build();
    ...
    void ping_permissionDenied_throwsRuntimeException() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("ping-auth-test").build();
    ...
    void ping_success_returnsServerInfo() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("ping-success-test").build();

      Kubemq.PingResult mockPingResult =
  ...
  class CloseTests {
    ...
    void close_normalShutdown_completesSuccessfully() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("close-test").build();
    ...
    void close_nullChannel_doesNotThrow() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("close-null-test").build();
    ...
    void close_multipleCalls_doesNotThrow() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("close-multiple-test").build();
  ...
  class AuthInterceptorTests {
    ...
    void builder_withoutAuthToken_hasNullToken() {
      CQClient testClient =
          CQClient.builder().address("localhost:50000").clientId("no-auth-test").build();
