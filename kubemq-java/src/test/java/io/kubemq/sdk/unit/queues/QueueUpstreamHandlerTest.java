...
class QueueUpstreamHandlerTest {
  ...
  class SendQueuesMessageTests {
    ...
    void sendQueuesMessage_sendsMessageAndReceivesResult() throws Exception {
      ...
      CompletableFuture<QueueSendResult> resultFuture =
          CompletableFuture.supplyAsync(() -> handler.sendQueuesMessage(message));
    ...
    void sendQueuesMessage_autoConnectsIfNotConnected() throws Exception {
      ...
      Kubemq.QueuesUpstreamResponse response =
          Kubemq.QueuesUpstreamResponse.newBuilder()
              .setRefRequestID(refRequestId)
              .addResults(
                  Kubemq.SendQueueMessageResult.newBuilder().setMessageID("msg-123").build())
              .build();
