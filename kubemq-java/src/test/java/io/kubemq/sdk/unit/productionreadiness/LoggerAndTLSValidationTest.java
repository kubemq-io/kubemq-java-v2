 ...
 *
 * <p>HIGH-3 FIX: setLogLevel() now modifies the "io.kubemq.sdk" logger instead of ROOT, so it only
 * affects SDK logging, not the entire application.
 *
 * <p>HIGH-4 FIX: TLS validation is now enabled, rejecting invalid TLS configurations (e.g., cert
 * without key, TLS enabled without proper configuration).
 *
...
class LoggerAndTLSValidationTest {
  ...
  @Order(1)
  @DisplayName(
      "HIGH-3 FIX: Creating SDK client does not modify ROOT logger (setLogLevel is deprecated no-op)")
  ...
  @Order(2)
  @DisplayName(
      "HIGH-3 FIX: SDK log level does not affect application loggers (setLogLevel is deprecated no-op)")
  void sdkLogLevel_doesNotAffectApplicationLoggers() {
    ch.qos.logback.classic.Logger appLogger = loggerContext.getLogger("com.mycompany.myapp");

    appLogger.setLevel(Level.DEBUG);
    assertEquals(
        Level.DEBUG,
        appLogger.getEffectiveLevel(),
  ...
  void verifySetLogLevel_modifiesSdkLogger() {
    ...
    try {
      ...
      assertEquals(rootBefore, rootAfter, "ROOT logger should NOT have been modified");
  ...
  @Order(4)
  @DisplayName(
      "HIGH-3 FIX: Multiple clients with different log levels don't affect ROOT (setLogLevel is deprecated)")
  ...
  void tlsEnabledWithoutCerts_isAllowed() {
    ...
    try {
      ...
      assertTrue(
          e.getMessage().contains("ALPN") || e.getMessage().contains("mutual TLS"),
  ...
  void tlsWithCertButNoKey_shouldThrow() {
    ...
    assertTrue(
        ex.getMessage().contains("tlsCertFile")
            || ex.getMessage().contains("tlsKeyFile")
  ...
  void tlsWithBothCertAndKey_isValid() {
    ...
    try {
      ...
      assertTrue(
          e.getMessage().contains("exist") || e.getMessage().contains("not found"),
  ...
  void combinedSecurityConfiguration_worksCorrectly() {
    ...
    try {
      ...
      try {
        assertEquals(rootBefore, rootLogger.getLevel(), "ROOT logger should not change");

        assertTrue(client.isTls(), "TLS should be enabled");
  ...
  void validateTlsConfiguration_methodExists() throws Exception {
    ...
    assertNotNull(validateMethod, "TLS validation method should exist in KubeMQClient");
  }
}
