 ...
 *
 * <p>HIGH-1 FIX: Synchronous methods like sendQueuesMessage() now use .get(timeout, TimeUnit) and
 * return a timeout result instead of blocking indefinitely.
 *
 * <p>HIGH-2 FIX: Pending responses now have a cleanup mechanism using ScheduledExecutorService that
 * removes stale entries after REQUEST_TIMEOUT_MS.
 *
...
class HandlerTimeoutAndCleanupTest {
  ...
  void sendQueuesMessage_returnsTimeoutResult() throws Exception {
    ...
    QueueMessage message =
        QueueMessage.builder().channel("test-queue").body("test".getBytes()).build();
    ...
    assertTrue(
        elapsed < 5000, "sendQueuesMessage should return within timeout. Took " + elapsed + "ms");

    // Result should indicate timeout/error
    assertNotNull(result, "Result should not be null");
    assertTrue(result.isError(), "Result should indicate error on timeout");
    assertTrue(
        result.getError().contains("timeout") || result.getError().contains("timed out"),
        "Error message should mention timeout. Got: " + result.getError());
  }

  @Test
  @DisplayName(
      "HIGH-1 FIX: receiveQueuesMessages returns timeout result instead of blocking forever")
  ...
  void receiveQueuesMessages_returnsTimeoutResult() throws Exception {
    ...
    doAnswer(invocation -> mock(StreamObserver.class)).when(mockAsyncStub).queuesDownstream(any());
  ...
  void upstreamHandler_hasCleanupExecutor() throws Exception {
    Field cleanupExecutorField = QueueUpstreamHandler.class.getDeclaredField("CLEANUP_EXECUTOR");
  ...
  void upstreamHandler_hasRequestTimestamps() throws Exception {
    ...
    assertTrue(
        timestamps instanceof ConcurrentHashMap, "requestTimestamps should be a ConcurrentHashMap");
  }
  ...
  void downstreamHandler_hasCleanupExecutor() throws Exception {
    Field cleanupExecutorField = QueueDownstreamHandler.class.getDeclaredField("CLEANUP_EXECUTOR");
    cleanupExecutorField.setAccessible(true);
  ...
  void downstreamHandler_hasRequestTimestamps() throws Exception {
    ...
    assertTrue(
        timestamps instanceof ConcurrentHashMap,
  ...
  void handlers_haveTimeoutConstant() throws Exception {
    ...
    Field downstreamTimeoutField =
        QueueDownstreamHandler.class.getDeclaredField("REQUEST_TIMEOUT_MS");
  ...
  void pendingResponses_areCleanedUp() throws Exception {
    ...
    @SuppressWarnings("unchecked")
    Map<String, Long> timestamps = (Map<String, Long>) timestampsField.get(upstreamHandler);
  ...
  void combinedFix_timeoutAndCleanup() throws Exception {
    ...
    assertTrue(
        result.getError().contains("timed out") || result.getError().contains("timeout"),
