...
/**
 * Unit tests for KubeMQClient builder, configuration, and validation. These tests use concrete
 * implementations (CQClient, PubSubClient, QueuesClient) to test the common KubeMQClient
 * functionality.
 *
 * <p>Note: Tests that require actual gRPC connections are in integration tests.
 */
class KubeMQClientTest {
  ...
  class BuilderValidationTests {
    ...
    void builder_withMinimalConfig_createsClient() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("test-client").build();
    ...
    void builder_withNullAddress_usesDefault() {
      CQClient client = CQClient.builder().address(null).clientId("test-client").build();
      assertNotNull(client);
      assertEquals("localhost:50000", client.getAddress());
      client.close();
    }
  ...
  class CloseTests {
    ...
    void close_multipleTimess_doesNotThrow() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("close-test-client").build();
  ...
  class GetterTests {
    ...
    void getAddress_returnsConfiguredAddress() {
      CQClient client =
          CQClient.builder().address("custom-host:12345").clientId("test").tls(false).build();
    ...
    void getClientId_returnsConfiguredId() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("my-unique-client-id").build();
    ...
    void getClient_returnsBlockingStub() {
      CQClient client = CQClient.builder().address("localhost:50000").clientId("test").build();
  ...
  class DifferentClientTypesTests {
    ...
    void pubSubClient_builderWorks() {
      PubSubClient client =
          PubSubClient.builder().address("localhost:50000").clientId("pubsub-test").build();
    ...
    void queuesClient_builderWorks() {
      QueuesClient client =
          QueuesClient.builder().address("localhost:50000").clientId("queues-test").build();
  ...
  class AuthTokenTests {
    ...
    void builder_withoutAuthToken_hasNullMetadata() {
      CQClient client =
          CQClient.builder().address("localhost:50000").clientId("no-auth-client").build();
