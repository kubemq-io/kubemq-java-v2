 ...
 *
 * <p>Each test is written to FAIL against the current (buggy) code and PASS once the corresponding
 * fix is applied. The finding ID is noted in every {@code @DisplayName}.
 */
...
class HighMediumFindingsTest {
  ...
  class H3_CommandMessageReceivedTagsDropped {
    ...
    void decode_shouldPreserveTagsFromProto() {
      ...
      assertTrue(
          decoded.getTags().containsKey("app-tag"),
  ...
  class H4_ReconnectHardCappedAt10 {
    ...
    void maxReconnectAttempts_shouldBeGreaterThan10OrConfigurable() {
      try {
        ...
        assertTrue(
            maxAttempts > 10,
            "MAX_RECONNECT_ATTEMPTS should be > 10 for production use, but was: " + maxAttempts);
      } catch (NoSuchFieldException e) {
        // If field is removed, check for constructor-based config
        boolean hasConfigurableMax =
            Arrays.stream(SubscriptionReconnectHandler.class.getConstructors())
  ...
  class H5_WithHandleBypassesErrorMapping {
    ...
    @Test
    @DisplayName(
        "H5: subscribeToEventsWithHandle should have same error handling as subscribeToEvents")
    void subscribeToEventsWithHandle_shouldHaveErrorMapping() throws Exception {
      ...
      Method withHandle =
          PubSubClient.class.getDeclaredMethod(
              "subscribeToEventsWithHandle", EventsSubscription.class);
      Method regular =
          PubSubClient.class.getDeclaredMethod("subscribeToEvents", EventsSubscription.class);
  ...
  class H6_SendResponseNoErrorMapping {
    ...
    @Test
    @DisplayName(
        "H6: CQClient.sendResponseMessage should map StatusRuntimeException to KubeMQException")
    void sendResponseMessage_shouldMapGrpcErrors() throws Exception {
      ...
      Method method =
          io.kubemq.sdk.cq.CQClient.class.getDeclaredMethod(
  ...
  class H7_ValidateThrowsWrongException {
    ...
    @Test
    @DisplayName(
        "H7: validate() with null commandReceived should throw ValidationException, not IllegalArgumentException")
    void validate_nullCommandReceived_shouldThrowValidationException() {
      CommandResponseMessage response =
          CommandResponseMessage.builder().commandReceived(null).build();
  ...
  class H8_OkStatusThrowsIllegalArgument {
    ...
    @Test
    @DisplayName(
        "H8: map() with Status.OK should return KubeMQException, not throw IllegalArgumentException")
    void map_statusOk_shouldReturnKubeMQException() {
      StatusRuntimeException okError =
          Status.OK.withDescription("Should not happen").asRuntimeException();

      // Currently throws IllegalArgumentException instead of returning KubeMQException
      try {
        KubeMQException result = GrpcErrorMapper.map(okError, "testOp", "testChannel", null, false);
        assertNotNull(result, "Should return a KubeMQException for Status.OK");
        assertInstanceOf(KubeMQException.class, result);
      } catch (IllegalArgumentException e) {
        fail(
            "GrpcErrorMapper.map() should NOT throw IllegalArgumentException for "
                + "Status.OK -- it should return a KubeMQException. Got: "
                + e.getMessage());
      }
    }
  }
  ...
  class H9_InternalStatusNotRetryable {
    ...
    void map_internal_shouldBeRetryable() {
      StatusRuntimeException internalError =
          Status.INTERNAL.withDescription("Server internal error").asRuntimeException();

      KubeMQException result = GrpcErrorMapper.map(internalError, "op", "ch", null, false);
  ...
  class H10_AbortedMappedToConnectionException {
    ...
    void map_aborted_shouldBeRetryableAndTransient() {
      StatusRuntimeException abortedError =
          Status.ABORTED.withDescription("Transaction aborted").asRuntimeException();

      KubeMQException result = GrpcErrorMapper.map(abortedError, "op", "ch", null, false);
      ...
      assertEquals(
          io.kubemq.sdk.exception.ErrorCategory.TRANSIENT,
          result.getCategory(),
  ...
  class H11_QueueAckSilentFailure {
    ...
    void ackAll_whenSendSilentlyDrops_shouldThrowOrReturnError() {
      ...
      RequestSender silentDropSender =
          request -> {
  ...
  class H12_TimeoutReturnsResultInsteadOfThrowing {
    ...
    @Test
    @DisplayName(
        "H12: sendEventStoreMessage should throw KubeMQTimeoutException on timeout, not return error result")
    void sendEventStoreMessage_timeout_shouldThrowKubeMQTimeoutException() throws Exception {
      ...
      assertTrue(result.getError().contains("timed out"), "Error message should mention timeout");
    }
  }
  ...
  @Nested
  @DisplayName(
      "H13: Inconsistent timestamp types between CommandMessageReceived and QueryMessageReceived")
  ...
  class M2_EventTimestampNeverPopulated {
    ...
    void decode_shouldPopulateTimestamp() {
      ...
      assertNotNull(
          ...
          "Timestamp should be populated from proto EventReceive.timestamp, " + "but was null");
    }
  }
  ...
  class M3_InstantCastToInt2038Overflow {
    ...
    @Test
    @DisplayName(
        "M3: EventsStoreSubscription with year 2040 start time should not overflow to negative")
    void encode_year2040_shouldNotOverflow() {
      ...
      assertTrue(
          epochSec > Integer.MAX_VALUE, "Year 2040 epoch seconds should exceed Integer.MAX_VALUE");
      ...
      assertEquals(
          epochSec, storeTypeValue, "eventsStoreTypeValue should exactly match the epoch second");
    }
  }
  ...
  class M5_ReconnectHandlerNotVolatile {
    ...
    void reconnectHandler_shouldBeVolatile() {
      try {
        ...
        fail("Could not find 'reconnectHandler' field in EventsSubscription: " + e.getMessage());
      }
    }
    ...
    void eventsStoreReconnectHandler_shouldBeVolatile() {
      try {
        ...
        fail(
            "Could not find 'reconnectHandler' field in EventsStoreSubscription: "
                + e.getMessage());
      }
    }
  }
  ...
  class M7_DuplicateIsActiveAccessors {
    ...
    @Test
    @DisplayName(
        "M7: PubSubChannel boolean isActive should use isActive() accessor, not getIsActive()")
    void pubSubChannel_shouldUseIsActiveAccessor() {
      ...
      // not getIsActive()
      boolean hasGetIsActive =
          Arrays.stream(PubSubChannel.class.getDeclaredMethods())
              .anyMatch(
                  m ->
                      m.getName().equals("getIsActive")
    ...
    void messages_shouldBeLong() {
      try {
        ...
        assertEquals(
            long.class,
            field.getType(),
            "PubSubStats.messages should be long but is: " + field.getType().getSimpleName());
      } catch (NoSuchFieldException e) {
    ...
    void volume_shouldBeLong() {
      try {
        ...
        assertEquals(
            long.class,
            field.getType(),
            "PubSubStats.volume should be long but is: " + field.getType().getSimpleName());
      } catch (NoSuchFieldException e) {
  ...
  class M11_TimeoutMultiplicationOverflow {
    ...
    @Test
    @DisplayName(
        "M11: encode() with large timeout should not produce negative value from int overflow")
    void encode_largeTimeout_shouldNotOverflow() {
      ...
      assertTrue(
          ...
          "Timeout in proto should be positive but got: "
              + protoTimeout
              + " (int overflow from 3_000_000 * 1000)");
    }
  }
  ...
  class M13_RetryThrottledContradictory {
    ...
    void throttlingCategory_shouldImplyRetryable() {
      RetryThrottledException exception =
          RetryThrottledException.builder()
