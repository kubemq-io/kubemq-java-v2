...
/**
 * Integration tests for server-side Dead Letter Queue (DLQ) functionality.
 * Requires a running KubeMQ server at localhost:50000 (or configured via KUBEMQ_ADDRESS).
 *
 * Validates that messages sent with attemptsBeforeDeadLetterQueue and deadLetterQueue
 * are automatically routed to the DLQ by the server after N rejections.
 */
...
class DlqIntegrationTest extends BaseIntegrationTest {
    ...
    void teardown() {
        if (client != null) {
            try { client.deleteQueuesChannel(testChannel); } catch (Exception ignored) {}
            try { client.deleteQueuesChannel(dlqChannel); } catch (Exception ignored) {}
            client.close();
        }
    }
    ...
    void serverSideDlq_shouldRouteAfterMaxRejections() {
        ...
        try {
            ...
            QueuesPollRequest mainPoll = QueuesPollRequest.builder()
                    .channel(channel)
                    .pollMaxMessages(1)
            ...
            assertTrue(mainResponse.getMessages().isEmpty(),
            ...
            try { client.deleteQueuesChannel(channel); } catch (Exception ignored) {}
            try { client.deleteQueuesChannel(dlq); } catch (Exception ignored) {}
        }
    }
    ...
    void serverSideDlq_shouldNotRouteToDlqIfAcked() {
        ...
        try {
            ...
            QueuesPollResponse dlqResponse = client.receiveQueuesMessages(dlqPoll);
            assertTrue(dlqResponse.getMessages().isEmpty(),
    ...
    void serverSideDlq_shouldIncrementReceiveCount() {
        ...
        try {
            ...
            for (int expected = 1; expected <= 2; expected++) {
                ...
                QueueMessageReceived received = response.getMessages().get(0);
                assertEquals(expected, received.getReceiveCount(),
                        "Receive count should be " + expected);
                received.reject();
                sleep(500);
            }
