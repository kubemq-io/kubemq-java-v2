...
/**
 * Integration tests for CQClient (Commands and Queries). Requires a running KubeMQ server at
 * localhost:50000 (or configured via KUBEMQ_ADDRESS).
 */
          ...
          QueriesSubscription.builder()
              ...
              .onReceiveQueryCallback(
                  qry -> {
                    receivedQuery.set(qry);
                    QueryResponseMessage response =
                        QueryResponseMessage.builder().queryReceived(qry).isExecuted(true).build();
                    responderClient.sendResponseMessage(response);
                  })
              .build();
