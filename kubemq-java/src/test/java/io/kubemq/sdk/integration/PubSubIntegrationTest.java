...
/**
 * Integration tests for PubSubClient. Requires a running KubeMQ server at localhost:50000 (or
...
class PubSubIntegrationTest extends BaseIntegrationTest {
  ...
  void sendEventStoreMessage_shouldReturnResult() {
    ...
    try {
      EventStoreMessage message =
          EventStoreMessage.builder().channel(channel).body("store test".getBytes()).build();
      ...
      assertTrue(
          ...
          "Send should not have error: " + result.getError());
  ...
  void groupSubscription_shouldLoadBalance() throws InterruptedException {
    ...
    try {
      ...
      for (int i = 0; i < 5; i++) {
        EventMessage message =
            EventMessage.builder().channel(channel).body(("message " + i).getBytes()).build();
        client.sendEventsMessage(message);
