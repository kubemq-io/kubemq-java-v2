...
/**
 * Integration tests for QueuesClient. Requires a running KubeMQ server at localhost:50000 (or
 * configured via KUBEMQ_ADDRESS).
 */
...
class QueuesIntegrationTest extends BaseIntegrationTest {
  ...
  void sendMessage_shouldSucceed() {
    ...
    QueueMessage message =
        QueueMessage.builder().channel(testChannel).body(messageBody.getBytes()).build();
  ...
  void sendWithTags_shouldPreserveTags() {
    ...
    try {
      ...
      QueueMessage message =
          QueueMessage.builder().channel(channel).body("test".getBytes()).tags(tags).build();
      client.sendQueuesMessage(message);
  ...
  void manualAck_shouldRequireExplicitAck() {
    ...
    try {
      // Send a message
      QueueMessage message =
          QueueMessage.builder().channel(channel).body("ack test".getBytes()).build();
      client.sendQueuesMessage(message);
  ...
  void rejectMessage_shouldReturnToQueue() {
    ...
    try {
      // Send a message
      QueueMessage message =
          QueueMessage.builder().channel(channel).body("reject test".getBytes()).build();
      client.sendQueuesMessage(message);
  ...
  void waitingMessages_shouldReturnPendingCount() {
    ...
    try {
      ...
      for (int i = 0; i < 3; i++) {
        QueueMessage message =
            QueueMessage.builder().channel(channel).body(("waiting test " + i).getBytes()).build();
        client.sendQueuesMessage(message);
      }
  ...
  void visibilityTimeout_shouldReturnMessageAfterExpiry() {
    ...
    try {
      // Send a message
      QueueMessage message =
          QueueMessage.builder().channel(channel).body("visibility test".getBytes()).build();
      client.sendQueuesMessage(message);
  ...
  void clientReconnects_shouldRecoverAfterDisconnection() {
    ...
    try {
      ...
      QueueMessage message1 =
          QueueMessage.builder().channel(channel).body("before reconnect".getBytes()).build();
      ...
      QueueMessage message2 =
          QueueMessage.builder().channel(channel).body("after reconnect".getBytes()).build();
