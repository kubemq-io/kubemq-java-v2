...
public class QueryMessage {
    ...
    public static class QueryMessageBuilder {
        public QueryMessage build() {
            ...
            QueryMessage message = new QueryMessage(id, channel, metadata,
                effectiveBody, effectiveTags, timeoutInSeconds, cacheKey, cacheTtlInSeconds);
    ...
    public void validate() {
        ...
        if ((metadata == null || metadata.isEmpty()) && (body == null || body.length == 0) && (tags == null || tags.isEmpty())) {
            throw ValidationException.builder()
                ...
                .message("Query message must have at least one of the following: metadata, body, or tags.")
        ...
        if (body.length > MAX_BODY_SIZE) {
            throw ValidationException.builder()
                .code(ErrorCode.INVALID_ARGUMENT)
                .message("Query message body size exceeds the maximum allowed size of " + MAX_BODY_SIZE + " bytes.")
                .operation("QueryMessage.validate")
                .channel(channel)
                .build();
        }
