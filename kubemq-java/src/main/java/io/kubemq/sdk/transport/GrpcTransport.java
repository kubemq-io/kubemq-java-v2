 ...
 * This is the ONLY class in the transport package that creates gRPC channels and stubs.
 *
 * <p>Interceptors (auth, error mapping, etc.) are built internally from
 * {@link TransportConfig} -- no gRPC types leak outside this package.</p>
 */
class GrpcTransport implements Transport {
    ...
    private void initChannel() {
        ...
        Channel channel = interceptors.isEmpty()
            ? managedChannel
    ...
    private void applyKeepAlive(ManagedChannelBuilder<?> builder) {
        int keepaliveTime = config.getKeepAliveTimeSeconds() == 0 ? 10 : config.getKeepAliveTimeSeconds();
        int keepaliveTimeout = config.getKeepAliveTimeoutSeconds() == 0 ? 5 : config.getKeepAliveTimeoutSeconds();
