...
public class QueueSendResult {
  ...
  /**
   * Decodes the given SendQueueMessageResult and sets the attributes of the QueueSendResult
    ...
    this.sentAt =
        result.getSentAt() > 0
            ? LocalDateTime.ofInstant(
                Instant.ofEpochSecond(result.getSentAt() / 1000000000), ZoneId.systemDefault())
            : null;
    this.expiredAt =
        result.getExpirationAt() > 0
            ? LocalDateTime.ofInstant(
                Instant.ofEpochSecond(result.getExpirationAt() / 1000000000),
                ZoneId.systemDefault())
            : null;
    this.delayedTo =
        result.getDelayedTo() > 0
            ? LocalDateTime.ofInstant(
                Instant.ofEpochSecond(result.getDelayedTo() / 1000000000), ZoneId.systemDefault())
            : null;
