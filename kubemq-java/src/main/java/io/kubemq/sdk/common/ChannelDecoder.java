...
/**
 * ChannelDecoder provides utility methods for decoding channel data from byte arrays. This class
 * uses Jackson's ObjectMapper to convert JSON byte arrays into lists of channel objects.
 */
@Internal
public class ChannelDecoder {
   ...
   *
   * @param dataBytes The byte array containing JSON data representing a list of PubSubChannel
   *     objects.
  ...
  public static List<PubSubChannel> decodePubSubChannelList(byte[] dataBytes) throws IOException {
    ...
    return OBJECT_MAPPER.readValue(dataStr, new TypeReference<List<PubSubChannel>>() {});
  }

  /**
   * Decodes a byte array into a list of QueuesChannel objects.
   *
   * @param dataBytes The byte array containing JSON data representing a list of QueuesChannel
   *     objects.
  ...
  public static List<QueuesChannel> decodeQueuesChannelList(byte[] dataBytes) throws IOException {
    String dataStr = new String(dataBytes, StandardCharsets.UTF_8);
    return OBJECT_MAPPER.readValue(dataStr, new TypeReference<List<QueuesChannel>>() {});
  }
  ...
  public static List<CQChannel> decodeCqChannelList(byte[] dataBytes) throws IOException {
    String dataStr = new String(dataBytes, StandardCharsets.UTF_8);
    return OBJECT_MAPPER.readValue(dataStr, new TypeReference<List<CQChannel>>() {});
  }
}
