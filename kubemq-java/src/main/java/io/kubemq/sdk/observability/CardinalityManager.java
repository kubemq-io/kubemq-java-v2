 ...
 *
 * <p>When unique channel count exceeds the configured threshold:
 *
 * <ol>
 *   <li>Emits a WARN log (once)
 *   <li>Omits {@code messaging.destination.name} from new metric series
 *   <li>Channels in the allowlist are always included regardless of threshold
 * </ol>
 */
final class CardinalityManager {
  ...
  CardinalityManager(CardinalityConfig config) {
    this.config = config;
    this.logger = KubeMQLoggerFactory.getLogger("io.kubemq.sdk.observability.CardinalityManager");
  }
    ...
    if (observedChannels.size() >= config.getMaxChannelCardinality()) {
      if (thresholdWarningEmitted.compareAndSet(false, true)) {
        logger.warn(
            ...
            "threshold",
            config.getMaxChannelCardinality(),
            "action",
            "omitting messaging.destination.name for new channels",
            "hint",
            "Configure channelAllowlist or increase maxChannelCardinality");
      }
      return false;
    }
