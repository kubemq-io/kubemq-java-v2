...
public class EventStoreMessage {
  ...
  /**
   * Target channel name for this persistent event. Must not be null or empty. Channels are created
  ...
  public static class EventStoreMessageBuilder {
    /**
     * Builds a validated {@link EventStoreMessage} from this builder's state.
     *
     * @return a new EventStoreMessage instance
     */
    public EventStoreMessage build() {
      ...
      EventStoreMessage message =
          new EventStoreMessage(id, channel, metadata, effectiveBody, effectiveTags);
  ...
  public void validate() {
    ...
    if (body.length > MAX_BODY_SIZE) {
      throw ValidationException.builder()
          .code(ErrorCode.INVALID_ARGUMENT)
          .message(
              "Event Store message body size exceeds the maximum allowed size of "
                  + MAX_BODY_SIZE
                  + " bytes.")
          .operation("EventStoreMessage.validate")
          .channel(channel)
  ...
  public String toString() {
    return String.format(
        "EventStoreMessage: id=%s, channel=%s, metadata=%s, body=%s, tags=%s",
