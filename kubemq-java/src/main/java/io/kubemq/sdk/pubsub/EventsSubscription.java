 ...
 * sequentially (one at a time) in the order messages are received from the server.</p>
 *
 ...
 * processing. When enabled, messages may be processed out of order.</p>
 *
 * <p><b>Blocking:</b> Do not perform long-running or blocking operations inside
 * callbacks. If heavy processing is needed, submit work to a separate worker pool.</p>
 */
@ThreadSafe
@Getter
@Setter
...
public class EventsSubscription {
     ...
     * executor (single-thread, sequential). Set to a custom executor to control
     * thread pool size for callback processing.
     */
    @Builder.Default
     ...
     * Default: 1 (sequential processing). When &gt; 1, callbacks may fire out of order.
     */
    @Builder.Default
            ...
            public void onError(Throwable t) {
                ...
                if (t instanceof io.grpc.StatusRuntimeException) {
                    KubeMQException mapped = GrpcErrorMapper.map(
                        (io.grpc.StatusRuntimeException) t, "subscribeToEvents", channel, null, false);
                    raiseOnError(mapped);
