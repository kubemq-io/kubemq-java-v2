...
public class EventStoreMessageReceived {
   ...
   *
   * <p>Converts the fields of the provided `EventReceive` object into the corresponding fields of
   * the `EventStoreMessageReceived` instance.
   *
  ...
  public static EventStoreMessageReceived decode(Kubemq.EventReceive eventReceive) {
    ...
    message.setTimestamp(
        eventReceive.getTimestamp() / 1000000000L); // Convert nanoseconds to seconds
  ...
  public String toString() {
    return String.format(
        "EventStoreMessageReceived: id=%s, channel=%s, metadata=%s, body=%s, fromClientId=%s, timestamp=%s, sequence=%d, tags=%s",
        id,
        channel,
        metadata,
