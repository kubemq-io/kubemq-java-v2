...
/**
 * Manages credential lifecycle: caching, serialized retrieval, reactive invalidation, and proactive
 * refresh scheduling.
 *
...
public class CredentialManager {
   ...
   * @param scheduler executor for scheduling proactive refresh (may be null to disable proactive
   *     refresh)
   */
  ...
  /**
   * Returns the current token, fetching from the provider if no cached token exists.
   *
  ...
  /** Invalidates the cached token. Called on UNAUTHENTICATED response for reactive refresh. */
  ...
  private String refreshToken() {
    ...
    try {
      ...
      LOG.debug("Token fetched, token_present: true, expiresAt: {}", result.getExpiresAt());
  ...
  private void scheduleProactiveRefresh(TokenResult result) {
    ...
    Duration buffer = Duration.ofMillis((long) (lifetime.toMillis() * REFRESH_BUFFER_FRACTION));
  ...
  /** Shuts down the proactive refresh scheduler. Call during client close. */
